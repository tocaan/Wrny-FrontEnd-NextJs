...
import { FaFacebookF, FaInstagram, FaLinkedin, FaTimes, FaTimesCircle, FaTwitter } from 'react-icons/fa';
...
export default function Footer() {
    ...
    const clearOffcanvasArtifacts = useCallback(() => {
      ...
      const closeMobileMenu = useCallback(async () => {
        if (typeof window === 'undefined') return;
        const el = document.getElementById('mobileMenuDrawer');
        if (!el) return;
        ...
        try {
          ...
          if (inst) {
            el.addEventListener('hidden.bs.offcanvas', clearOffcanvasArtifacts, { once: true });
            inst.hide();
          } else {
            el.classList.remove('show');
            el.setAttribute('aria-hidden', 'true');
            clearOffcanvasArtifacts();
          }
        } catch {
          el.classList.remove('show');
          el.setAttribute('aria-hidden', 'true');
      ...
      useEffect(() => {
        clearOffcanvasArtifacts();
      }, [pathname, clearOffcanvasArtifacts]);
    ...
    return (
        <div>
            ...
            >
                <div className="offcanvas-header d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" className="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"><FaTimes color='#000' size={18} /></button>
                </div>

                <div className="offcanvas-body">
                    {/* Links */}
                    <div className="list-group list-group-flush mb-4">
                        <Link href="/" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/', { exact: true }) ? 'active' : ''}`}>
                            {t('home')}
                        </Link>
                        <Link href="/about" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/about', { exact: true }) ? 'active' : ''}`}>
                            {t('about')}
                        </Link>
                        <Link href="/categories" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/categories') ? 'active' : ''}`}>
                            {t('categories')}
                        </Link>
                        <Link href="/companies" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/companies') ? 'active' : ''}`}>
                            {t('companies')}
                        </Link>
                        <Link href="/events" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/events') ? 'active' : ''}`}>
                            {t('events')}
                        </Link>
                        <Link href="/contact" onClick={closeMobileMenu} className={`list-group-item list-group-item-action rounded-2 mb-2 py-2 ${isActive('/contact', { exact: true }) ? 'active' : ''}`}>
                            {t('contact')}
                        </Link>
                    </div>
        ...
        <button className="btn btn-outline-primary" onClick={toggleLanguage}>
            ...
            {lang === 'ar' ? 'English' : 'العربية'}
                    ...
                    <div className="d-lg-flex justify-content-center align-items-center py-3 text-center">
                        <div className="text-black text-primary-hover">
                            تم التصميم والتطوير بواسطة{' '}
                            <Link target='_blank' href="https://tocaan.com" className="text-primary">
                                شركة توكان
                            </Link>
                        </div>
                    </div>
